...
internal abstract class Actor(string name, int glyph, Color color, int maxHP, int defense, int power, 
    int capacity) : Entity(name, glyph, color, EntityLayer.Actors, false, true)
{
    int _hp;

    public int MaxHP { get; init; } = maxHP;
    public int Defense { get; init; } = defense;
    public int Power { get; init; } = power;
    public Inventory Inventory { get; init; } = new(capacity);
