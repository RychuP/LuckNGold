    ...
    new ComponentTypeTagPair(typeof(ItemList<RoomPath>), "Paths"))
{
    ...
    protected override IEnumerator<object?> OnPerform(GenerationContext context)
    {
        ...
        int maxAttempts = 100; // mainPath.Count / 2;
        for (int i = 0; i < maxAttempts; i++)
        {
            ...
            CreateRooms(ref sidePath, ref corridors, GetRooms(context),
                startRoom, roomCount, mapBounds);

            if (sidePath.Count == 0)
                continue;

            // update context
            AddRoomsToContext(context, sidePath);
        }
